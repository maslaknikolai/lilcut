...
import { libraryOrderAtom, projectsAtom, videosAtom } from '@/App/atoms'
...
export function ProjectRemoveModal({ projectId, onClose }: ProjectRemoveModalProps) {
  ...
  const usedVideos = videos.filter((video) => usedOpfsNames.includes(video.opfsName))
  ...
  return (
    <Modal
      ...
      onClose={onClose}
    >
      {!usedVideos.length && <span className="text-sm text-slate-400">This project uses no videos.</span>}
